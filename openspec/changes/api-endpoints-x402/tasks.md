...
- [x] 2.2 Implement `POST /generate` — validate request body (contractAddress, chainId required), generate UUID, create S3 placeholder, fire pipeline async, return `{ id, url }` (S3 and pipeline stubbed as TODO — awaiting storage/pipeline layers)
- [x] 2.3 Implement `GET /:id` — fetch from S3 via `getSkill`, return content as text/markdown or 404 (S3 stubbed as TODO — returns 404 until storage layer is implemented)
